...
class AliensEnvPygame(AliensEnv):
    def __init__(self, level=0, render=False, flag_AI=True):
        super().__init__(level=level, render=render)
        self.frames = []
        self.log_folder = f'logs/PlayDatas/game_records_lvl{level}_{self.timing}'
        if not flag_AI:
    ...
    def save_gif(self, filename='replay.gif',path=None, duration=0.1):
        if(path==None):#手动传递存储gif的路径
...
def main():
    ...
    env = AliensEnvPygame(level=0, render=False,flag_AI=False)
