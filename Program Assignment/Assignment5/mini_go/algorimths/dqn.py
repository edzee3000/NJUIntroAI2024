...
#导入必要的Python模块，包括collections、random、os、numpy、sonnet和tensorflow。
...
#定义了两个命名元组Transition和StepOutput，分别用于存储智能体在环境中的转换（即经验）和智能体的输出。
...
#定义了一个常量，用于在计算Q值时对非法动作的惩罚。
ILLEGAL_ACTION_LOGITS_PENALTY = -1e9

#定义了一个ReplayBuffer类，它是一个固定大小的先入先出（FIFO）经验回放缓冲区，用于存储智能体的经验并支持从中均匀采样。
...
class DQN:
    ...
    定义了一个DQN类，它是DQN智能体的具体实现。具体使用TensorFlow的框架
    """
    def __init__(self,
                 session,
                 player_id,
        ...
        """Initialize the DQN agent.
         构造函数__init__初始化了智能体的各种参数，包括学习率、隐藏层大小、经验回放缓冲区等，并构建了Q网络和目标Q网络。"""
    ...
    def _epsilon_greedy(self, info_state, legal_actions, epsilon):
        ...
        _epsilon_greedy方法实现了ε-贪婪策略，用于在探索和利用之间进行权衡。
        """
    ...
    def learn(self):
        ...
        learn方法实现了DQN的学习过程，包括从经验回放缓冲区中采样经验、计算损失和更新Q网络。
        """
    ...
    @property
    def q_values(self):
        """DQN类还包含了一些属性，如q_values、replay_buffer、info_state_ph、loss等，用于访问智能体的内部状态。"""
