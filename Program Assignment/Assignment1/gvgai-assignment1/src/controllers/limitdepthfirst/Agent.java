...
public class Agent extends AbstractPlayer {
    ...
    protected int max_limit_depth=10;//设置最大限制深度
    ...
    boolean get_LDFS_Actions(StateObservation stateObs, ElapsedCpuTimer elapsedTimer,int depth){
        ...
        for(int i=0;i<all_available_actions.size();i++){                                             
            ...
            if(test_past_state(stCopy) || stCopy.isGameOver()) Actions.remove(Actions.size() - 1);  //如果动作施加后的状态之前已经到达过或者游戏失败就尝试下一个动作，并且需要删除当前已经加进去的这个动作
                ...
                if(get_LDFS_Actions(stCopy,elapsedTimer,depth+1))  return true;               //递归进行深度优先搜索   
    ...
    public Types.ACTIONS act(StateObservation stateObs, ElapsedCpuTimer elapsedTimer) {
        ...
        // System.out.println();               
        if(!have_path){
            have_path = get_LDFS_Actions(stateObs, elapsedTimer,0);           //如果没有搜索过则进行搜索，搜索成功后将flag置为true，并且已将路径存储在Actions中
        }
