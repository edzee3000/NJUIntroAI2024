...
public class ArcadeMachine
{
     ...
     * @param game_file game description file.游戏描述文件
     * @param level_file file with the level to be played.包含要播放的级别。
     * @param visuals true to show the graphics, false otherwise.表示显示图形，否则为false。
     * @param agentName name (inc. package) where the controller is otherwise.名称（包括包），否则为控制器。
     * @param actionFile filename of the file where the actions of this player, for this game, should be recorded.件的文件名，应记录该玩家在该游戏中的操作
     * @param randomSeed sampleRandom seed for the sampleRandom generator.生成器的随机种子。
     * @param isHuman indicates if a human is playing the game. 指示是否有人在玩游戏
     */
                                    ...
                                    String agentName, String actionFile, int randomSeed, boolean isHuman)
    {
        ...
        // First, we create the game to be played..//创建即将要玩的游戏
